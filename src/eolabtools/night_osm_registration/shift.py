...
# Interpolation method for OTB GridBasedImageResampling
INTERPOLATOR = "nn"
KEEP_OTB_FILES = False



def apply_displacement_grid_scipy1(input_raster, displacement_grid, output_raster,
                                 target_width, target_height, interpolation='nearest'):
    """
    Apply grid-based transformation to input raster using GDAL/rasterio

    Args:
        input_raster: Path to input raster file
        displacement_grid: Path to displacement grid file (VRT format with X,Y bands)
        output_raster: Path to output raster file
        target_width: Output raster width in pixels
        target_height: Output raster height in pixels
        interpolation: Interpolation method ('bilinear', 'nearest', 'cubic', etc.)
    """
    # Read input raster
    ...
    # Read displacement grid (assuming 2-band grid with X,Y displacements)
    with rasterio.open(displacement_grid) as grid:
        x_displacement = grid.read(1)  # X displacement
        y_displacement = grid.read(2)  # Y displacement
        grid_transform = grid.transform

    # Create coordinate meshgrid for output
    cols, rows = np.meshgrid(np.arange(target_width), np.arange(target_height))

    # Create coordinate arrays for output grid
    output_transform = rasterio.transform.from_bounds(
        *rasterio.transform.array_bounds(target_height, target_width, input_transform),
    ...
    # Interpolate displacement grid to target resolution if needed
    if x_displacement.shape != (target_height, target_width):
        from scipy.ndimage import zoom
        scale_y = target_height / x_displacement.shape[0]
        scale_x = target_width / x_displacement.shape[1]
        x_displacement = zoom(x_displacement, (scale_y, scale_x), order=1)
        y_displacement = zoom(y_displacement, (scale_y, scale_x), order=1)

    # Apply displacement directly in pixel coordinates
    # Assuming displacement is in pixel units
    displaced_cols = cols.astype(np.float64) + x_displacement
    displaced_rows = rows.astype(np.float64) + y_displacement

    # Clip to valid pixel range to avoid transform errors
    displaced_cols = np.clip(displaced_cols, 0, input_data.shape[2] - 1)
    displaced_rows = np.clip(displaced_rows, 0, input_data.shape[1] - 1)

    # Apply transformation to each band
    output_data = np.zeros((input_data.shape[0], target_height, target_width),
                           dtype=input_data.dtype)

    for band in range(input_data.shape[0]):
        output_data[band] = map_coordinates(
            input_data[band],
            [displaced_rows, displaced_cols],
            order=3,
            mode='constant',
            cval=0,
            prefilter=False  # Avoid potential numerical issues
        )

    # Write output raster with same transform as input (since we're working in pixel space)
    with rasterio.open(
            output_raster, 'w',
            driver='GTiff',
...
def apply_displacement_grid_scipy2(otb_input, otb_displacementgrid, otb_output):
    # Read input raster (all bands)
    with rasterio.open(otb_input) as new_dataset:
        raster = new_dataset.read()  # shape: (bands, rows, cols)
        profile = new_dataset.profile

    # Read displacement grid (assumed to be two bands: dx and dy)
    with rasterio.open(otb_displacementgrid) as grid_src:
        dx = grid_src.read(1)
        dy = grid_src.read(2)
    ...
    # Prepare coordinates for map_coordinates (shape: (2, rows*cols))
    coords = np.vstack((source_y.ravel(), source_x.ravel()))

    # Prepare output array
    warped = np.empty_like(raster, dtype=np.float32)

    # Interpolate each band independently
    for b in range(bands):
        band_data = raster[b]
        warped_band = map_coordinates(band_data, coords, order=1, mode='nearest')
        warped[b] = warped_band.reshape((rows, cols))

    # Update profile to float32 (optional, depends on input type)
    profile.update(dtype=rasterio.float32)

    # Write output raster
    with rasterio.open(otb_output, 'w', **profile) as dst:
        dst.write(warped.astype(rasterio.float32))
...
def apply_shift(infile: Path, disp_grid: np.ndarray, outdir: Path):
    ...
    # app.SetParameterString("io.in", str(otb_input))
    # app.SetParameterString("grid.in", str(otb_displacementgrid))
    # app.SetParameterString("io.out", str(otb_output))
    # app.SetParameterInt("out.sizex", raster_src.width)
    # app.SetParameterInt("out.sizey", raster_src.height)
    # app.SetParameterString("grid.type", "def")
    # app.SetParameterString("interpolator", "bco")
    # app.ExecuteAndWriteOutput()

    # # Usage example (equivalent to your OTB code)
    # apply_displacement_grid_scipy1(
    #     input_raster=str(otb_input),
    #     displacement_grid=str(otb_displacementgrid),
    #     output_raster=str(otb_output),
    #     target_width=raster_src.width,
    #     target_height=raster_src.height,
